...
static char* CLASS_NAME = (char*)"SaveLoad";
...
void SaveLoad::fillListBox() {
	...
	HANDLE hFind = FindFirstFile("MishMash*.dat", &fdata);
...
char* SaveLoad::createFilename() {
	...
	// MishMash_YYMMDDHHMMSS.dat
	strftime(buffer, 127, "MishMash_%y%m%d%H%M%S.dat", &timeinfo);

	return buffer;
}

// user has pressed 'S'. Save current parameter settings to file.
void SaveLoad::saveParam() {
...
void SaveLoad::loadParam(int index) {
...
const char* SaveLoad::displayString(const char *filename) {
	...
	sprintf_s(str, 127, "MishMash %02d/%02d/%2d %02d:%02d:%02d", day, month, year, hour, minute, second);
	return str;
}
